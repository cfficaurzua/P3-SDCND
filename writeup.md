...
In order to collect data, I used the simulator provided by Udacity with a mouse as an input, I realized that there were different quality settings that range from fastest to fantastic, when I inspected the output of the images taken, I notice immediately that they were differences, for example in the fantastic settigs there were shadows and sharp edges, not present in the fastest settins, therefore, with the intention of generalize the model,  I recorded 2 laps back and forth in both tracks with each setting, furthermore I recorded an additional set of laps driving in the left lane as is driven in England or India.
...
The data retrieved by the simulator was naturallly unbalanced, there were many samples with steering angles near zero, and few samples of closed curves near  -1 and 1, so I created a function that selects randomly samples until no more than a threshold per angle has been completed.
...
The input image to the model was first normalized, shrinked to a range between -.5 and .5, then the environment such as trees and sky were cropped along with the hood of the car, using the embedded function Cropping2d from the keras library.

## Model

After trying with different architectures, including leNet, nVidia end-to-end I chose the architecture shared by comma.ai, and tweak it a little bit, adding a firts 1x1 convolution layer to help the model understand the color space, adding a Dense layer of 10, before the last layer, an changing some parameters as is shown in the following table.
...
To compile the model an Adam optimizer was used, with a learning rate of 1e-4, but then was change to 1e-5, the loss function chosen was the mean absolute value, as was well spread in the slack channel to have much better results.

## L2 Regularizacion

At first using the n-vidia model the car drove pretty well in the second track, but run dreadfully in the first track, in order to examine what was going on, I decided to take a look inside at the activations on the second-last and third-last layers.
To perform the analysis I insert as an input a collection of images with more or less the same steering angle, going from left to right, and then taking the median per neuron of all activations. Looking at the result I immediately notice surprisingly, that only 2 of the 10 neurons were relevant for the second-last layer, and no more tahn 4 of the 50 neurons in the third-last layer.
...
Unfortunately this plan failed to succeed, the car drove only straight ahead without turns and brake every couple of seconds, probably because retrieving the steering angles was a bottleneck in the system.
